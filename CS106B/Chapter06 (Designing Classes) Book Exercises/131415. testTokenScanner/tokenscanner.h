...
* Method: skipToken
* --------------------------------------
* Throws away the next token whithout returning it as a result
*/
    void skipToken();
...
* Method: peekToken
* --------------------------------------
* returns the next token without removing it from the scanner object
*/
...
* Method: reinsert
* --------------------------------------
* Reinserts argument to the begining of the buffer. Useful after nextToken
* has passed a token that needs to go back to the buffer
*/
    void reinsertToken(std::string str);
...
* next Token method treats whitespace characters (typically spaces and
...
/* Instance variables */
    ...
    bool ignorePunctuationFlag; /* Flag set by a call to ignorePunctuation */
    bool ignoreNumbersFlag;     /* Flag set by a call to ignoreNumbers */
    bool scanStringsFlag;     /* Flag set by a call to scanStrings */
    bool scanNumbersFlag;     /* Flag set by a call to scanNumbers */

    Stack<std::string> sBuffer;


/* Private methods */
void skipPreferences();
void fillStack();
std::string numberextractor();
